...
public class ListObjects extends StoreEntryPoint {
  ...
  public static final String USAGE = "Usage: listobjects <path>\n" + STANDARD_OPTS
      + optusage(DELETE, "delete the objects") + optusage(LIMIT, "limit", "limit of files to list")
      + optusage(PURGE, "purge directory markers") + optusage(QUIET, "quiet output");

  public ListObjects() {
    createCommandFormat(1, 1, PURGE, DELETE, QUIET);
  ...
  public int run(String[] args) throws Exception {
    ...
    try (StoreDurationInfo duration = new StoreDurationInfo(LOG, "listobjects")) {
      ...
      ListObjectsV2Request request =
          S3ListingSupport.createListObjectsV2Request(source.toUri().getHost(), key, null);

      final ObjectListingIterator objects = new ObjectListingIterator(s3, source, request);
      List<String> prefixes = new ArrayList<>();
      int objectCount = 0;
      long size = 0;
      List<ObjectIdentifier> objectsToDelete = new ArrayList<>(deletePageSize);
      ...
      while (!finished && objects.hasNext()) {
        ...
        for (S3Object summary : page.contents()) {
          ...
          if (!quiet) {
            ...
            println("[%05d] %s%s", objectCount, S3ListingSupport.stringify(summary), extra);
          }
          ...
          if (delete) {
            ...
            if (objectsToDelete.size() >= deletePageSize) {
              final List<ObjectIdentifier> keyVersions = objectsToDelete;
      ...
      if (!markers.isEmpty()) {
        ...
        List<ObjectIdentifier> kv = new ArrayList<>(markerCount);
        ...
        if (purge) {
          ...
          if (!delete) {
            println("\nTo delete these markers, rerun with the option -%s", PURGE);
          }
        }
      } else if (purge) {
   ...
   * @param kv
   */
  public void delete(S3Client s3, String bucket, List<ObjectIdentifier> kv) throws IOException {
    if (kv.isEmpty()) {
      return;
    }
    try (
   ...
   */
  public boolean objectRepresentsDirectory(S3Object summary) {
