...
class ObjectListingIterator implements RemoteIterator<ListObjectsV2Response> {
  ...
  /**
   * Count of how many listings have been requested (including initial result).
   */
   ...
   * if there was a problem talking to the bucket.
   * 
   * @param listPath path of the listing
   * @param request initial request to make
   */
  ObjectListingIterator(S3Client s3, Path listPath, ListObjectsV2Request request) {
  ...
  /**
   * Declare that the iterator has data if it is either is the initial iteration or it is a later
   * one and the last listing obtained was incomplete.
   * 
  ...
  /**
   * Ask for the next listing. For the first invocation, this returns the initial set, with no
   * remote IO. For later requests, S3 will be queried, hence the calls may block or fail.
   * 
  ...
  public String toString() {
    return "Object listing iterator against " + listPath + "; listing count " + listingCount
