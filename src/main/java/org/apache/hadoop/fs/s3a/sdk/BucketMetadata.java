...
public class BucketMetadata extends StoreEntryPoint {
  ...
  public static final String USAGE = "Usage: bucketmetadata [-debug] <path>";
  ...
  public int run(String[] args) throws Exception {
    ...
    if (!(fs instanceof S3AFileSystem)) {
      println("Filesystem for path %s is not an S3A FileSystem %s", path, fs);
      return -1;
    }
    ...
    println("Region %s%nLocation Name %s%nLocation Type %s%n", response.bucketRegion(),
        response.bucketLocationName(), response.bucketLocationTypeAsString());
