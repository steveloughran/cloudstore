...
public class BulkDeleteCommand extends StoreEntryPoint {
    ...
    public static final String USAGE = "Usage: bulkdelete [-verbose] [-page <pagesize>] <path> <file>\n"
    ...
    public int run(String[] args) throws Exception {
        ...
        final List<Path> pathsToDelete = toDelete.stream()
                .map(l -> fs.makeQualified(new Path(l)))
                .filter(p -> !p.isRoot())
        ...
        heading("Summary");
        println("Bulk delete of %d file(s) finished, duration: %s", files, executionDuration.getDurationString());
        println("Batch count: %d. Failure count: %d", batches, failures);
        if (isVerbose()) {
            heading("Statistics");
            println("\nStatistics\n%s", ioStatisticsToPrettyString(retrieveIOStatistics(fs)));
        }
