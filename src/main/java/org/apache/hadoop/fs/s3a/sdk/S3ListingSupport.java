...
public final class S3ListingSupport {
   ...
   * @return the request
   */
  public static ListObjectsV2Request createListObjectsV2Request(final String bucket, String key,
      String delimiter) {
   ...
   * @return the request
   */
  public static ListObjectVersionsRequest createListVersionsRequest(final String bucket, String key,
      String delimiter) {
    ListObjectVersionsRequest.Builder request =
        ListObjectVersionsRequest.builder().bucket(bucket).maxKeys(MAX_KEYS).prefix(key);
    if (delimiter != null) {
      request.delimiter(delimiter);
    }
    return request.build();
  ...
  public static boolean objectRepresentsDirectory(final String name, final long size) {
    boolean hasDirName = !name.isEmpty() && name.charAt(name.length() - 1) == '/';

    boolean isEmpty = size == 0L;
    if (hasDirName && !isEmpty) {
      LOG.warn("Warning: object {} has length {} so is not a directory marker", name, size);
    }
  ...
  public static String stringify(S3Object summary) {
    return String.format("\"%s\"\tsize: [%d]\t%s\ttag: %s", summary.key(), summary.size(),
        summary.lastModified().toString(), summary.eTag());
  }
