...
public class ListMultiparts extends StoreEntryPoint {
    ...
    public int run(String[] args) throws Exception {
        ...
        try (StoreDurationInfo duration = new StoreDurationInfo(getOut(), name)) {
            ...
            int entries = 0;
            csv.columns("index", "key", "size", "blocks", "date", "id");
            csv.newline();
            long totalParts = 0;
            long totalSize = 0;
            while (uploads.hasNext()) {
                ...
                final MultipartProcessor.PartIterator listing = multipartProcessor.partListing(key, upload.uploadId());
            ...
            println();
            println(
                    "Found %,d uploads under %s with total size %,d bytes in %,d parts",
