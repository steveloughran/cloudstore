...
public class MkCSV extends StoreEntryPoint {
  ...
  public static final String USAGE =
      "Usage: mkcsv\n" + STANDARD_OPTS + optusage(HEADER, "print a header row")
          + optusage(QUOTE, "quote column text") + "\t<records> <path>";
  ...
  /**
   * number of numeric elements in column 2. Width of the column will be at most ELEMENTS * 5;
   */
  ...
  public MkCSV() {
    createCommandFormat(2, 2, HEADER, QUOTE);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    long rows = (long) getDataSize(size, StorageUnit.BYTES);
    if (rows < 0) {
    ...
    try (StoreDurationInfo d = new StoreDurationInfo(LOG, "Opening %s for writing", path)) {
      upload = fs.createFile(path).progress(progress).recursive().bufferSize(BUFFER_SIZE)
          .overwrite(true).build();
    }
    try {
      CsvWriterWithCRC writer = new CsvWriterWithCRC(upload, SEPARATOR, EOL, quote);
      if (header) {
        writer.columns(START, "rowId", "length", "dataCrc", "data", "rowId2", "rowCrc", END);
        writer.newline();
      }
