...
final class UploadEntry implements Serializable, Comparable<UploadEntry> {
  ...
  public enum State {
    ready, queued, active, skipped, succeeded, failed
  }
  ...
  /**
   * Destination path. Need not be qualified for dest FS, but must be absolute.
   */
  private Path dest;
  ...
  public boolean isCompleted() {
    return state == State.succeeded || state == State.failed || state == State.skipped;
  }
  ...
  /**
   * Equality checks (final) source field only. {@inheritDoc}
   */
  @Override
  ...
  /**
   * Hash code uses checks (final) source field only. {@inheritDoc}
   */
  @Override
  public int hashCode() {
   ...
   * 
   * @param o other entry
   * @return -1 if this object it smaller than the other, 1 if it is the other way around, and 0 if
   *         they are equal.
   */
  @Override
  ...
  public String toString() {
    final StringBuilder sb = new StringBuilder("UploadEntry{");
