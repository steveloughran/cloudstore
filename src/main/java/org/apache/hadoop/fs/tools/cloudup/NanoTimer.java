...
/**
 * A simple class for timing operations in nanoseconds, and for printing some useful results in the
 * process.
 */
public final class NanoTimer {
  ...
  public long end(String format, Object... args) {
    long d = end();
    LOG.info("Duration of {}: {} nS", String.format(format, args), toHuman(d));
    return d;
  }
   ...
   * 
   * @param bytes bytes in
   * @return the number of bits per second this operation. 0 if duration == 0.
   */
  ...
  /**
   * Get a description of the bandwidth, even down to fractions of a MB.
   * 
  ...
  /**
   * Log the bandwidth of a timer as inferred from the number of bytes processed.
   * 
   * @param timer timer
   * @param bytes bytes processed in the time period
   */
  public static void bandwidth(NanoTimer timer, long bytes) {
    LOG.info("Bandwidth = {}  MB/S", timer.bandwidthDescription(bytes));
  }
