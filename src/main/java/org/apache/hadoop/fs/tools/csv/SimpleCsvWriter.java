...
/**
 * Write CSV files with quoting of strings, booleans mapped to 1/0.
 */
public class SimpleCsvWriter implements Closeable {
  ...
  /**
   * track whether the write is the start of the line. if it isn't, a separator is inserted before
   ...
   */
  public SimpleCsvWriter(final Writer out, final String separator, final String eol,
   ...
   * @param closeOutput should the output be closed in close(), or just flushed?
   */
  public SimpleCsvWriter(final OutputStream out, final String separator, final String eol,
      final boolean quote, final boolean closeOutput) {
  ...
  /**
   * Write a bool, mapping true to 1 and false to 0. That makes counting the number of matched rows
   * straightforward.
   * 
   * @param b boolean
   * @return this instance.
