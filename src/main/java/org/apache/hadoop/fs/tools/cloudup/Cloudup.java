...
public class Cloudup extends StoreEntryPoint {
    ...
    public int run(String[] args) throws Exception {
        ...
        if (destFS.equals(sourceFS)) {
            ...
            StoreUtils.checkArgument(!s.startsWith(d), "Source path " + s + "%s is under destination path " + d);
            StoreUtils.checkArgument(!d.startsWith(s), "Destination path " + s + "%s is under source path " + d);
        }

        // worker pool
        workers = new ThreadPoolExecutor(threads, threads, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>());
        ...
        println(
                "Largest %,d " + COPIES + " commenced, total size = %,d bytes. Remaining files: %,d",
                submittedFiles,
                sortUploadSize,
                remaining);
        ...
        if (remaining > 0) {
            ...
            println("Shuffled files and queued: %,d, total size = %,d bytes", shuffledUploadCount, shuffledUploadSize);
        }
        ...
        if (filesActuallyUploaded > 0) {
            println(
                    "Effective bandwidth %,.3f MiB/s, %,.3f Megabits/s",
                    uploadTimer.bandwidth(finalUploadedSize), uploadTimer.bandwidthMegabits(finalUploadedSize));
            println("Seconds per file: %.3fs", ((double) (uploadDuration.value()) / (filesActuallyUploaded * 1000)));
        } else {
    ...
    private Outcome uploadOneFile(final UploadEntry upload) {
        ...
        try {
            println(
                    "[%s] [%04d] " + COPYING + " %s to %s (size: %,d bytes)",
                    threadId,
                    upload.getId(),
                    source,
                    dest,
            ...
            final String outcome = state == UploadEntry.State.succeeded
                    ...
                    : ("Skipped " + COPY_LC + " of");
            println(
                    "[%s] [%04d] %s %s to %s  (size: %,d bytes) in %ss",
                    threadId,
                    upload.getId(),
                    outcome,
                    source,
            ...
            upload.setEndTime(now());
            println("[%s] [%04d] Failed to " + COPY_LC + " %s to %s: %s", threadId, upload.getId(), source, dest, e);
    ...
    private UploadEntry.State copyFile(final UploadEntry upload, final Path dest)
        ...
        try (FSDataInputStream in = await(input.build());
            ...
            try (StoreDurationInfo d = new StoreDurationInfo(LOG, isVerbose(), "[%04d] close writer (%s)", id, dest)) {
                out.flush();
                out.close();
            }
            if (verbose) {
    ...
    private static final class Outcome {
        ...
        private Outcome(
                final boolean executed, final UploadEntry upload, final long bytesUploaded, final Exception exception) {
