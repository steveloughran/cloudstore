...
public class SafePrefetch extends StoreEntryPoint {
    ...
    public static final String USAGE = "Usage: safeprefetch [options] <path>\n" + STANDARD_OPTS;
    ...
    public int run(String[] args) throws Exception {
        ...
        if (!checker.hasPathCapability(abfsPath, etag_capability)) {
            println(
                    "Filesystem is not from a release with the prefetch issue (no path capability %s)",
                    etag_capability);
            return 0;
        }
        if (checker.hasPathCapability(abfsPath, readahead_safe)) {

            println("Filesystem %s has prefetch issue fixed (has path capability %s)", abfsPath, readahead_safe);
            return 0;
        }
        ...
        if (fsConf.getInt(FS_AZURE_READAHEADQUEUE_DEPTH, 2) == 0) {
            println("Queue depth is zero so prefetching will not take place (%s = 0)", FS_AZURE_READAHEADQUEUE_DEPTH);
            return 0;
        }
