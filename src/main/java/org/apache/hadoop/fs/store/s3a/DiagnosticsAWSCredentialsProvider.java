...
public class DiagnosticsAWSCredentialsProvider implements AwsCredentialsProvider {

    private static final Logger LOG = LoggerFactory.getLogger(DiagnosticsAWSCredentialsProvider.class);

    private static final String[] secrets = {"fs.s3a.access.key", "fs.s3a.secret.key", "fs.s3a.session.token"};
     ...
     */
    public void printSecretOption(final String key) {
        String source = "";
        String option;
        char[] password;
        try {
        ...
        if (password != null) {
            ...
            LOG.info("Option {} = {} {} from {}", key, sanitize(option), md5sum(password), source);
        } else {
    ...
    private void printEnvVar(String var) {
        ...
        if (value != null) {
            LOG.info("Env Var {} = {} {}", var, sanitize(value), md5sum(value.getBytes(StandardCharsets.UTF_8)));
        }
    }
