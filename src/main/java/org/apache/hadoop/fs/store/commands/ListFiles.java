...
/**
 * ListFiles recursive scan of a directory tree, with some size limiting to avoid going overboard on
 * a superdeep tree.
 *
...
public class ListFiles extends StoreEntryPoint {
  ...
  public static final String USAGE =
      "Usage: list <path>\n" + STANDARD_OPTS + optusage(LIMIT, "limit", "limit of files to list");
  ...
  public int run(String[] args) throws Exception {
    ...
    heading("Listing%s files under %s", limit == 0 ? "" : (" up to " + limit), source);
    ...
    final RemoteIterator<LocatedFileStatus> lister = fs.listFiles(source, true);
    try {
    ...
    println("Data size %s (%,d bytes)", byteCountToDisplaySize(totalSize), totalSize);
    println("Mean file size %s (%,d bytes)", byteCountToDisplaySize(bytesPerFile), bytesPerFile);
    if (largestFile != null) {
      println("Largest file: %s: size %s", largestFile.getPath(),
