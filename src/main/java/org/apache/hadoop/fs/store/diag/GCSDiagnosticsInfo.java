...
public class GCSDiagnosticsInfo extends StoreDiagnosticsInfo {
  ...
  public static final String SERVICE_ACCOUNT_PRIVATE_KEY = "fs.gs.auth.service.account.private.key";
   ...
   */
  private static final Object[][] options = {{"fs.gs.auth.type", false, false},
      ...
      {"fs.gs.auth.service.account.json.keyfile", true, false}, {"fs.gs.client.id", true, false},
      {"fs.gs.client.secret", true, true}, {"fs.gs.application.name.suffix", false, false},
      {"fs.gs.auth.client.id", false, false}, {"fs.gs.auth.client.secret", true, true},
      ...
      {"fs.gs.batch.threads", false, false}, {"fs.gs.block.size", false, false},
      {"fs.gs.bucket.delete.enable", false, false}, {"fs.gs.checksum.type", false, false},
      ...
      {"fs.gs.encryption.algorithm", false, false}, {"fs.gs.encryption.key", false, false},
      {"fs.gs.encryption.key.hash", false, false}, {"fs.gs.glob.algorithm", false, false},
      {"fs.gs.grpc.checksums.enable", false, false}, {"fs.gs.grpc.enable", false, false},
      {"fs.gs.grpc.read.metadata.timeout.ms", false, false},
      {"fs.gs.grpc.read.timeout.ms", false, false}, {"fs.gs.grpc.server.address", false, false},
      {"fs.gs.grpc.write.buffered.requests", false, false},
      {"fs.gs.grpc.write.timeout.ms", false, false}, {"fs.gs.http.connect-timeout", false, false},
      {"fs.gs.http.max.retry", false, false}, {"fs.gs.http.read-timeout", false, false},
      ...
      {"fs.gs.lazy.init.enable", false, false}, {"fs.gs.list.max.items.per.call", false, false},
      {"fs.gs.marker.file.pattern", false, false}, {"fs.gs.max.requests.per.batch", false, false},
      {"fs.gs.max.wait.for.empty.object.creation.ms", false, false},
      {"fs.gs.operation.move.enable", false, false},
      ...
      {"fs.gs.outputstream.upload.cache.size", false, false}, {"fs.gs.project.id", false, false},
      {"fs.gs.proxy.address", false, false}, {"fs.gs.proxy.password", false, true},
      {"fs.gs.proxy.username", false, false}, {"fs.gs.reported.permissions", false, false},
      {"fs.gs.requester.pays.buckets", false, false}, {"fs.gs.requester.pays.mode", false, false},
      ...
      {"fs.gs.status.parallel.enable", false, false}, {"fs.gs.storage.http.headers", false, false},
      {"fs.gs.storage.root.url", false, false}, {"fs.gs.storage.service.path", false, false},
      {"fs.gs.working.dir", false, false},
      {"fs.viewfs.overload.scheme.target.gs.impl", false, false},
      {"google.cloud.auth.client.file", false, false}, {"google.cloud.auth.client.id", true, false},
      ...
      {"fs.gs.implicit.dir.repair.enable", false, false}, {"", false, false},};

  protected static final Object[][] ENV_VARS = {{"DEVSHELL_CLIENT_POR", false},
      {"GCE_METADATA_HOST", false}, {"GOOGLE_APPLICATION_CREDENTIALS", true},
      {"GOOGLE_APPLICATION_CREDENTIALS_SKIP_APP_ENGINE", false},
      {"GOOGLE_CLOUD_QUOTA_PROJECT", false}, {"NO_GCE_CHECK", false},
      {"SUPPRESS_GCLOUD_CREDS_WARNING", false}, {"", false},};

  /**
   * Standard System properties.
   */
  public static final Object[][] SYSPROPS = {{"flogger.backend_factory", false}, {"", false},};
  ...
  /*
   * @Override public List<URI> listOptionalEndpointsToProbe(final Configuration conf) throws
   * IOException, URISyntaxException { List<URI> l = new ArrayList<>(0); l.add(new
   * URI("http://169.254.169.254")); return l; }
   */
