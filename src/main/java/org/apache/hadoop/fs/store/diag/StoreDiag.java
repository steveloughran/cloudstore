...
public class StoreDiag extends DiagnosticsEntryPoint {
  ...
  public void printSecurityState() throws IOException {
    ...
    if (tokenPath != null) {
      ...
      if (!tokenFile.exists()) {
        ...
        println("Token file updated on %s; age is %d:%02d hours", new Date(modified), ageInHours,
            minutes);
        if (ageInHours > 12) {
  ...
  /**
   * Probe one endpoint, print proxy values, etc. No auth. Ignores "0.0.0.0" addresses as they are
   * silly.
   * 
  ...
  public static void printCanonicalHostname(final Printout out, final String host)
    ...
    out.println("Canonical hostname %s%n  IP address %s", addr.getCanonicalHostName(),
        addr.getHostAddress());
  }

  /**
   * Probe all the required classes from base settings, the FS diags, and any file passed in as
   * -required.
   * 
  ...
  public void probeRequiredAndOptionalClasses(boolean optional)
    ...
    if (file != null) {
      ...
      probeRequiredClassesOrResources(org.apache.commons.io.IOUtils
          .readLines(Files.newInputStream(f.toPath()), Charset.forName("UTF-8")));
    }
  }

  /**
   * Take a list of lines containing: comments, classes, resources and possibly blankness, trip and
   * either probe for or print the comment.
   * 
  ...
  /**
   * Dump log4J files; special handling for the case that >1 ends up on the classpath.
   * 
   * @throws IOException IO failure.
   */
  public void dumpLog4J() throws IOException {
    ...
    println("%n%s", CharStreams
        .toString(new InputStreamReader(getClass().getResourceAsStream(resource), Charsets.UTF_8)));
  }
  ...
  public boolean executeFileSystemOperations(final Path baseDir,
    ...
    try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Examine root path", root)) {
      ...
      println("ls / contains %s entries; first entry %s", len,
          (len > 0 ? statusToString(rootListing[0]) : "n/a"));
      ...
      println(
          "  - filesystem is only visible to the account and wrong account was used: check account");
    ...
    if (serviceName == null) {
      println("Filesystem %s does not/is not configured to issue delegation tokens%s", fsUri,
          securityEnabled ? "" : " (at least while security is disabled)");
    } else {
      //
      Credentials cred = new Credentials();
      try (StoreDurationInfo ignored =
          new StoreDurationInfo(getOut(), "collecting delegation tokens")) {
        try {
    ...
    try (StoreDurationInfo ignored =
        new StoreDurationInfo(getOut(), "creating a directory %s", dir)) {
      fs.mkdirs(dir);
    ...
    try {
      ...
      try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Reading file %s", file)) {
        ...
        if (!HELLO.equals(utf)) {
          throw new StoreDiagException(
              "Expected %s to contain the text %s" + " -but it has the text \"%s\"", HELLO, file,
              utf);
        }
      } finally {
        closeStream(in);
      ...
      if (!userName.equals(status.getOwner())) {
        warn("Expected file owner to be %s but was reported as %s in %s", userName,
            status.getOwner(), status);
      }
      ...
      try {
        try {
          ...
          if (!out.hasCapability(OPTION_CREATE_IN_CLOSE)) {
            errorln("The stream does not declare that it supports %s for write to %s",
          ...
          try {
            ...
            println("The overwrite attempt was rejected *as expected* in close: %s",
                e.getMessage());
            LOG.debug("error", e);
          }
        } catch (IOException e) {
  ...
  private boolean readFile(final FileSystem fs, final FileStatus status) throws IOException {
    ...
    try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Reading file %s", path);
      ...
      int c = in.read();
      println("First character of file %s is 0x%02x: '%s'", path, c,
