...
public class StoreDiag extends DiagnosticsEntryPoint {
    ...
    public StoreDiag() {
        createCommandFormat(1, 1, DELEGATION, ENVARS, HIDE, JARS, LOGDUMP, MD5, OPTIONAL, READONLY, WRITE);
    ...
    public int run(String[] args, PrintStream stream) throws Exception {
        ...
        heading("Store Diagnostics for %s on %s", UserGroupInformation.getCurrentUser(), NetUtils.getHostname());
    ...
    public void tlsInfo() {
        ...
        lookupAndPrintSanitizedValues(TLS_SYSPROPS, "TLS System Properties", System::getProperty);
        println();
        try {
    ...
    public void printStoreConfiguration() throws IOException {
        ...
        printOptions("Selected Configuration Options", conf, storeInfo.getFilesystemOptions());
    }
    ...
    public void printSecurityState() throws IOException {
        ...
        heading("Security");
        println("Security Enabled: %s", UserGroupInformation.isSecurityEnabled());
        println("Keytab login: %s", UserGroupInformation.isLoginKeytabBased());
        println("Ticket login: %s", UserGroupInformation.isLoginKeytabBased());
        ...
        if (tokenPath != null) {
            ...
            if (!tokenFile.exists()) {
                ...
                println("Token file updated on %s; age is %d:%02d hours", new Date(modified), ageInHours, minutes);
                if (ageInHours > 12) {
                    warn("Token file is old: tokens may have expired");
                }
            }
        }
        Collection<Token<? extends TokenIdentifier>> tokens = currentUser.getTokens();
     ...
     * @return the store's diagnostics.
     */
    public StoreDiagnosticsInfo bindToStore(final URI fsURI) throws IOException {
        heading("Diagnostics for filesystem %s", fsURI);

        StoreDiagnosticsInfo store = StoreDiagnosticsInfo.bindToStore(fsURI, this);

        println("%s%n%s%n%s", store.getName(), store.getDescription(), store.getHomepage());
     ...
     * @throws IOException IO Failure
     */
    public void probeEndpoints(final List<EndpointProbe> probes) throws IOException {
        if (probes.isEmpty()) {
     ...
     * @throws IOException IO Failure
     */
    public void probeOptionalEndpoints(final List<EndpointProbe> probes) throws IOException {
        for (EndpointProbe probe : probes) {
            try {
                probeOneEndpoint(probe);
            } catch (Exception e) {
     ...
     */
    public void probeOneEndpoint(EndpointProbe probe) throws IOException {
        ...
        List<Proxy> proxies = ProxySelector.getDefault().select(toURI(url));
        if (proxies.isEmpty() || Proxy.Type.DIRECT == proxies.get(0).type()) {
        ...
        try (StoreDurationInfo d = new StoreDurationInfo(getOut(), "GET %s", url)) {
            conn.connect();
            responseCode = conn.getResponseCode();
            String responseMessage = conn.getResponseMessage();
            println("Response: %d : %s", responseCode, responseMessage);
            success = responseCode == 200;
            println("HTTP response %d from %s: %s", responseCode, url, responseMessage);
            ...
            if (headerFields != null) {
                for (String header : headerFields.keySet()) {
                    println("%s: %s", header, StringUtils.join(",", headerFields.get(header)));
                }
            }
            String body;
            ...
            println("%n%s%n", body.substring(0, Math.min(1024, body.length())));
            if (success && !probe.isExpectWorldReadable()) {
                println(
                        "An unauthenticated GET request to this endpoint was not rejected.%n "
                                + "This may mean the store is world-readable.%n "
                                + "Check this by pasting %s into your browser",
                        url);
            }
        } catch (IOException e) {
     ...
     */
    public static void printCanonicalHostname(final Printout out, final String host) throws UnknownHostException {
        InetAddress addr = InetAddress.getByName(host);
        out.println("Canonical hostname %s%n  IP address %s", addr.getCanonicalHostName(), addr.getHostAddress());
    }
     ...
     */
    public void probeRequiredAndOptionalClasses(boolean optional) throws ClassNotFoundException, IOException {
        ...
        if (file != null) {
            ...
            probeRequiredClassesOrResources(org.apache.commons.io.IOUtils.readLines(
                    Files.newInputStream(f.toPath()), Charset.forName("UTF-8")));
        }
    }
     ...
     */
    public void probeRequiredAndOptionalResources() throws ClassNotFoundException, IOException {
    ...
    private void probeForFileSystemClass(String scheme) throws IOException {
        ...
        try {
            Class<? extends FileSystem> clazz = FileSystem.getFileSystemClass(scheme, getConf());
            ...
            if (e instanceof UnsupportedFileSystemException || e.toString().contains("No FileSystem for scheme")) {
    ...
    public void dumpEnvVars() throws IOException {
        ...
        for (Map.Entry<String, String> entry : vars.entrySet()) {
            ...
            String value = maybeSanitize(entry.getValue(), key.startsWith("AWS_"));
    ...
    public void dumpLog4J() throws IOException {
        ...
        Enumeration<URL> logjJs = getClass().getClassLoader().getResources(resource);
        int found = 0;
        ...
        println(
                "%n%s",
                CharStreams.toString(new InputStreamReader(getClass().getResourceAsStream(resource), Charsets.UTF_8)));
    }
     ...
     */
    public boolean executeFileSystemOperations(final Path baseDir, final boolean attempWriteOperations)
            throws IOException {
        final Configuration conf = getConf();
        ...
        try (StoreDurationInfo ignored = new StoreDurationInfo(LOG, "Creating filesystem for %s", baseDir)) {
        ...
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Examine root path", root)) {
            ...
            println(
                    "ls / contains %s entries; first entry %s",
                    len, (len > 0 ? statusToString(rootListing[0]) : "n/a"));
        } catch (FileNotFoundException e) {
            errorln("%s failed: the remote filesystem doesn't seem to exist: %s", operation, root);
            ...
            throw new StoreDiagException(E_NOT_FOUND, "Store not found %s: %s", root, e.toString()).initCause(e);
        }
        ...
        try (StoreDurationInfo ignored =
                new StoreDurationInfo(getOut(), "First %d entries of listStatus(%s)", limit, baseDir)) {
            RemoteIterator<FileStatus> statuses = fs.listStatusIterator(baseDir);
            int statusCount = 0;
            baseDirFound = true;
            while (statuses.hasNext() && (limit > 0 || firstFile == null)) {
                statusCount++;
        ...
        limit = LIST_LIMIT;
        try (StoreDurationInfo ignored =
                new StoreDurationInfo(getOut(), "First %d entries of listFiles(%s)", limit, baseDir)) {
            ...
            try {
                ...
                LOG.warn("Permission denied during recursive scan of {}", baseDir, e);
            }
        ...
        if (serviceName == null) {
            println(
                    "Filesystem %s does not/is not configured to issue delegation tokens%s",
                    fsUri, securityEnabled ? "" : " (at least while security is disabled)");
        } else {
            //
            Credentials cred = new Credentials();
            try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "collecting delegation tokens")) {
                try {
            ...
            if (size > 0) {
                ...
                println("Filesystem %s %s issue any delegation tokens %s", fsUri, outcome, error);
            }
        }
        if (requireToken && !issued) {
            throw new StoreDiagException(E_NO_ACCESS, "No delegation token issued by filesystem %s", fsUri);
        }
        ...
        try (StoreDurationInfo ignored =
                new StoreDurationInfo(getOut(), "probe for a directory which does not yet exist %s", dir)) {
            FileStatus status = fs.getFileStatus(dir);
            println("Unexpectedly got the status of a file which should not exist%n" + "    %s", status);
        ...
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "creating a directory %s", dir)) {
            fs.mkdirs(dir);
        ...
        // Directory ops
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "create directory %s", dir)) {
            FileStatus status = fs.getFileStatus(dir);
        ...
        try {
            ...
            try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Creating file %s", file)) {
            ...
            try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "ListFiles(%s)", dir)) {
                final RemoteIterator<LocatedFileStatus> listing = fs.listFiles(dir, false);
                boolean found = false;
            ...
            try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Reading file %s", file)) {
                in = fs.open(file);
                ...
                if (!HELLO.equals(utf)) {
                    throw new StoreDiagException(
                            "Expected %s to contain the text %s" + " -but it has the text \"%s\"", HELLO, file, utf);
                }
            } finally {
                closeStream(in);
            }
            ...
            if (!userName.equals(status.getOwner())) {
                warn("Expected file owner to be %s but was reported as %s in %s", userName, status.getOwner(), status);
            }
            ...
            println(
                    "File modtime after creation = %,d millis,"
                            + "\n\tafter close invoked = %,d millis"
                            + "\n\tafter close completed = %,d millis",
                    offsetFromCreation, offsetFromClose, offsetFromCompletion);
            if (offsetFromCreation < 0) {
                warn("Timestamp of created file is %,d milliseconds before the local clock", offsetFromCreation);
            } else {
                println("Timestamp of created file is %,d milliseconds after the local clock", offsetFromCreation);
            ...
            try {
                try {
                    ...
                    if (!out.hasCapability(OPTION_CREATE_IN_CLOSE)) {
                        errorln(
                                "The stream does not declare that it supports %s for write to %s",
                                OPTION_CREATE_IN_CLOSE, out);
                    }
            ...
            try (StoreDurationInfo ignored =
                    new StoreDurationInfo(getOut(), "Renaming file %s under %s", file, subdir)) {
                fs.mkdirs(subdir);
     ...
     * Read a file.
     */
    private boolean readFile(final FileSystem fs, final FileStatus status) throws IOException {
        boolean accessWasDenied = false;
        Path path = status.getPath();
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "Reading file %s", path);
                FSDataInputStream in = fs.open(path)) {
            // read the first char or -1
            int c = in.read();
            println(
                    "First character of file %s is 0x%02x: '%s'",
                    path, c, (c > ' ') ? Character.toString((char) c) : "(n/a)");
            printStreamCapabilities(in, CapabilityKeys.INPUTSTREAM_CAPABILITIES);
    ...
    private void printStreamCapabilities(final StreamCapabilities in, final String[] capabilities) {
    ...
    public void deleteDir(final FileSystem fs, final Path dir) {
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "delete directory %s", dir)) {
            try {
                fs.delete(dir, true);
            } catch (Exception e) {
    ...
    protected void verifyPathNotFound(FileSystem fs, Path path) throws IOException {
        try (StoreDurationInfo ignored = new StoreDurationInfo(getOut(), "probing path %s", path)) {
            final FileStatus st = fs.getFileStatus(path);
            throw new StoreDiagException("Found path which should be absent %s", st);
     ...
     */
    public static int diagnostics(Configuration conf, String... args) throws Exception {
