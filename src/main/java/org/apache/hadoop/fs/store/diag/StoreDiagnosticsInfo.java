...
public class StoreDiagnosticsInfo {
     ...
     */
    public static StoreDiagnosticsInfo bindToStore(URI fsURI, final Printout output) throws IOException {
     ...
     */
    public Configuration patchConfigurationToInitalization(final Configuration conf) throws IOException {
        return conf;
    }
     ...
     * connections.
     */
    public List<EndpointProbe> listOptionalEndpointsToProbe(final Printout printout, Configuration conf)
            throws IOException, URISyntaxException {
        return EMPTY_ENDPOINTS;
    }
     ...
     */
    protected boolean addProbeFromOption(
            final List<EndpointProbe> uris,
            final Configuration conf,
            final String key,
            final String uriPrefix,
            final String description,
            final String defVal,
            boolean expectWorldReadable)
            throws IOException {
        String endpoint = conf.getTrimmed(key, defVal);
        if (!endpoint.isEmpty()) {
            uris.add(new EndpointProbe(
                    uriPrefix + endpoint, description, "From configuration key " + key, expectWorldReadable));
            return true;
        } else {
            return false;
        }
     ...
     */
    protected void validateConfig(Printout printout, final Configuration conf, final boolean writeOperations)
            throws IOException {}
     ...
     */
    protected void warnOnInvalidDomain(final Printout printout, final String domain, final String followupURL) {
        ...
        if (host == null) {
            ...
            printout.warn("The URL for this store normally contains the domain %s," + " but it is %s", domain, host);
     ...
     */
    protected void printPrefixedOptions(final Printout printout, final Configuration conf, final String prefix) {
     ...
     * @throws IOException
     */
    protected static void validateBufferDir(
        ...
        printout.println("Buffer configuration option %s = \"%s\"", bufferOption, bufferOptionUntrimmed);
        ...
        if (createTempFile) {
            ...
            File temp = directoryAllocator.createTmpFileForWrite("temp", fileSize > 0 ? fileSize : 1, conf);
     ...
     */
    protected boolean sizeHint(final Printout printout, final Configuration conf, String option, long recommend) {
    ...
    protected boolean timeHint(
        ...
        return hint(printout, shouldRecommend, "Option %s has value %d (source %s). %s", option, val, source, hintStr);
    }
    ...
    public void validateOutputStream(
            final Printout printout, final FileSystem fs, final Path file, final FSDataOutputStream data)
            throws IOException {}
     ...
     */
    void validateInputStream(final Printout printout, final FileSystem fs, final Path file, final InputStream in) {}
