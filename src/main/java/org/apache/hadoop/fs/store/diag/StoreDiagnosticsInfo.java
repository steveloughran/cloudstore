...
public class StoreDiagnosticsInfo {
  ...
  protected static final String[] DEFAULT_OPTIONAL_RESOURCES = {"log4j.properties"};
  ...
  public static StoreDiagnosticsInfo bindToStore(URI fsURI, final Printout output)
    ...
    if (scheme == null) {
      try {
        ...
        fsURI = new URI(defaultUri.getScheme(), defaultUri.getUserInfo(), defaultUri.getHost(),
            defaultUri.getPort(), fsURI.getPath(), fsURI.getQuery(), fsURI.getFragment());
  ...
  /**
   * List of env vars for filesystems. Each entry must be a pair of (string, sensitive); sensitive
   * strings don't have their values fully printed.
   * 
   * @return option array
   */
  ...
  /**
   * List of system properties. Each entry must be a pair of (string, sensitive); sensitive strings
   * don't have their values fully printed.
   * 
   * @return option array
   */
  ...
  /**
   * should HTTPS/TLS binding info be printed? default is true.
   * 
  ...
  /**
   * Take the raw config and patch as the FS will have during initialization. This handles stores
   * like S3A which do some per-bucket config.
   * 
  ...
  /**
   * Get a list of optional classes to look for. If any of these are not found, it is not an error.
   * 
   * @param conf config to use in determining their location.
  ...
  /**
   * Get a list of resources to look for. If any of these are not found, it is an error.
   * 
   * @param conf config to use in determining their location.
  ...
  /**
   * Get a list of optional resources to look for. If any of these are not found, it is not an
   * error.
   * 
   * @param conf config to use in determining their location.
   ...
   * @return a possibly empty ist of endpoints for DNS lookup and HTTP connections.
   */
  public List<EndpointProbe> listEndpointsToProbe(final Printout printout, Configuration conf)
      throws IOException, URISyntaxException {
    return EMPTY_ENDPOINTS;
  }

  /**
   * List optional endpoints to probe; its not an error if these aren't reachable.
   * 
   * @param printout
   * @param conf configuration to use, will already have been patched.
   * @return a possibly empty ist of endpoints for DNS lookup and HTTP connections.
  ...
  /**
   * Get a list of optional capablities to look for. If any of these are not true.
   * 
   ...
   */
  protected boolean addProbeFromOption(final List<EndpointProbe> uris, final Configuration conf,
      final String key, final String uriPrefix, final String description, final String defVal,
      boolean expectWorldReadable) throws IOException {
    String endpoint = conf.getTrimmed(key, defVal);
    if (!endpoint.isEmpty()) {
      uris.add(new EndpointProbe(uriPrefix + endpoint, description, "From configuration key " + key,
          expectWorldReadable));
      return true;
    } else {
      return false;
   ...
   */
  protected void validateConfig(Printout printout, final Configuration conf,
  ...
  /**
   * Warn if the Fs URI is in the wrong domain. It's tempting to fail fast here to stop people
   * missing it, but there's a risk that people are using a custom domain and they don't want a
   * failure.
   * 
   * @param printout dest for messages
   * @param domain domain to expect.
   */
  protected void warnOnInvalidDomain(final Printout printout, final String domain,
      final String followupURL) {
    final String host = getFsURI().getHost();
    if (host == null) {
      printout.warn("The URL For this store doesn't have a valid host %s", getFsURI());
    } else if (!host.endsWith(domain)) {
      printout.warn("The URL for this store normally contains the domain %s," + " but it is %s",
          domain, host);
      printout.warn("Unless you are using a private endpoint, this is NOT" + " GOING TO WORK");
      if (followupURL != null) {
   ...
   * configuration prefix. Property names in the mapping are trimmed to remove the configuration
   * prefix.
   * 
   * @param confPrefix configuration prefix
   * @return mapping of configuration properties with prefix stripped
   */
  public Map<String, String> getPropsWithPrefix(final Configuration conf, String confPrefix) {
  ...
  /**
   * Validate the buffer dir, up to and including creating a temp file of the chosen size.
   * 
   ...
   * @throws IOException
   */
  protected static void validateBufferDir(final Printout printout, final Configuration conf,
      final String bufferDirKey, final String fallbackDirKey, final boolean createTempFile,
      final long fileSize) throws IOException {

    printout.heading("Validating buffer directories");
    String bufferOption = conf.get(bufferDirKey) != null ? bufferDirKey : fallbackDirKey;

    final String bufferOptionUntrimmed = conf.get(bufferOption);
    final String bufferOptionRaw = conf.getRaw(bufferOption);
    printout.println("Buffer configuration option %s = \"%s\"", bufferOption,
        bufferOptionUntrimmed);
    ...
    while (!dirQueue.isEmpty()) {
      ...
      if (!absDir.exists()) {
        ...
        /*
         * final File parentFile = absDir.getParentFile(); if (parentFile != null &&
         * !parentFile.getCanonicalPath().equals("/")) { dirQueue.add(parentFile.getAbsolutePath());
         * } continue;
         */
    ...
    if (createTempFile) {
      ...
      final LocalDirAllocator directoryAllocator = new LocalDirAllocator(bufferOption);

      File temp =
          directoryAllocator.createTmpFileForWrite("temp", fileSize > 0 ? fileSize : 1, conf);

      printout.println("\nTemporary file successfully created in %s", temp.getParentFile());
      temp.delete();
    } else {
  ...
  /**
   * Any preflight checks of the filesystem config/url etc. Happens before the endpoint checks
   * -which are the first network operations.
   * 
   * @param printout output
   * @param path path which will be used
   * @throws IOException failure.
   */
  public void preflightFilesystemChecks(Printout printout, Path path) throws IOException {}

  /**
   * Perform any validation of the filesystem itself (is it the right type, are there any options
   * you can check for. This is called after the FS has been created, but before any read or write
   * IO has taken place. Important: if you try to cast the FS for these checks, then if the FS JARs
   * aren't on the classpath, the specific diagnostics info subclass will fail with a
   * ClassNotFoundException. Do not cast, at least not directly in that specific class.
   * 
   * @param printout dest for messages
   ...
   * @throws IOException failure
   */
  public void validateFile(Printout printout, FileSystem filesystem, Path path, FileStatus status)
      throws IOException {}
   ...
   * @param conf config
   */
  protected void performanceHints(Printout printout, Configuration conf) {}
   ...
   */
  protected boolean hint(final Printout printout, boolean condition, String text, Object... args) {
    if (condition) {
   ...
   */
  protected boolean sizeHint(final Printout printout, final Configuration conf, String option,
      long recommend) {

    // print a message if unset
    if (hint(printout, conf.get(option) == null,
        "Option %s is unset. Recommend a value of at least %d", option, recommend)) {
      return true;
    }
    // work out the origin
    ...
    return hint(printout, val < recommend,
        "Option %s (source %s) has value %d. Recommend a value of at least %d", option, source, val,
        recommend);
  }

  /**
   * Hint when the time is too low/unset, knowing that zero is a special case which is not hinted on
   * if {@code isZeroSpecial} is set.
   * <p>
   ...
   * @return true if a hint was made
   */
  protected boolean timeHint(final Printout printout, final Configuration conf, String option,
      Duration recommendDuration, boolean isZeroSpecial, String description) {
    ...
    // print a message if unset
    if (hint(printout, conf.get(option) != null,
        "Option %s is unset. Recommend a value of at least %dms", option, recommend)) {
      return true;
    }
    // if set, check the value
    ...
    return hint(printout, shouldRecommend, "Option %s has value %d (source %s). %s", option, val,
        source, hintStr);
  }
   ...
   */
  public void validateOutputStream(final Printout printout, final FileSystem fs, final Path file,
      final FSDataOutputStream data) throws IOException {}

  /**
   * Validate an open input stream. The stream is the wrapped stream from FSDataInputStream; its
   * type can be validated, cast to the fs-specific value, etc.
   * 
   * @param printout printout
   * @param fs filesystem
   * @param file filename
   ...
   */
  void validateInputStream(final Printout printout, final FileSystem fs, final Path file,
