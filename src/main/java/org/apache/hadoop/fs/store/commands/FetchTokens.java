...
public class FetchTokens extends StoreEntryPoint {
    ...
    protected Credentials saveTokens(Configuration conf, Path dest, String renewer, boolean required, List<String> urls)
        ...
        for (String url : urls) {
            ...
            try (StoreDurationInfo ignored = new StoreDurationInfo(LOG, "Fetching tokens for %s", path)) {
                ...
                if (tokens != null && tokens.length != 0) {
                    ...
                    if (required) {
                        throw new ExitUtil.ExitException(
                                LauncherExitCodes.EXIT_NOT_FOUND, "No tokens issued by filesystem " + fsUri);
                    }
                }
            }
        }
        ...
        try (StoreDurationInfo ignored =
                new StoreDurationInfo(LOG, "Saving %d token%s to %s", count, plural(count), dest)) {
