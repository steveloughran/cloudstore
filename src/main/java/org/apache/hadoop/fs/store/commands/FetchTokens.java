...
public class FetchTokens extends StoreEntryPoint {
  ...
  public int run(String[] args, PrintStream stream) throws Exception {
    ...
    println("Collecting tokens for %d filesystem%s to to %s", urls.size(), plural(urls.size()),
        dest);
    Credentials retrieved =
        self.doAs((PrivilegedExceptionAction<Credentials>) () -> saveTokens(conf, dest, renewer,
            required, urls));
  ...
  /**
   * Fetch and save the tokens; print their details. in a verbose run, the filesystem statistics are
   * also printed
   * 
   ...
   */
  protected Credentials saveTokens(Configuration conf, Path dest, String renewer, boolean required,
