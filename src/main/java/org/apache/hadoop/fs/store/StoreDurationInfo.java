...
public class StoreDurationInfo implements AutoCloseable {
  ...
  /**
   * Create the duration text from a {@code String.format()} code call and log either at info or
   * debug.
   * 
   * @param log log to write to
   ...
   * @param args list of arguments
   */
  public StoreDurationInfo(Logger log, boolean logAtInfo, String format, Object... args) {
    this.started = time();
  ...
  public StoreDurationInfo(PrintStream out, String format, Object... args) {
    ...
    if (out != null) {
      out.printf("[%s] Starting: %s%n", humanTime(started), getFormattedText());
    }
  }
  ...
  public static String humanTime(long time) {
    ...
    return String.format("%02d:%02d:%02d.%03d", hours % 24, minutes % 60, seconds % 60, millis);
  }
  ...
  /**
   * Close the duration, invoke {@link #finished()}. Log the final state if a log or output stream
   * was provided.
   */
  @Override
  public void close() {
