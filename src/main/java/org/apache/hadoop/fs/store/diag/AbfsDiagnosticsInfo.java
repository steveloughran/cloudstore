...
public class AbfsDiagnosticsInfo extends StoreDiagnosticsInfo {
     ...
     * {@value}
     */
    public static final String FS_AZURE_BLOCK_UPLOAD_ACTIVE_BLOCKS = "fs.azure.block.upload.active.blocks";
    ...
    public static final String FS_AZURE_INFINITE_LEASE_DIRECTORIES = "fs.azure.infinite-lease.directories";
    ...
    public static final String ACCESS_TOKEN_PROVIDER = "org.apache.hadoop.fs.azurebfs.oauth2.AccessTokenProvider";

    public static final String CREDENTIALS_TOKEN_PROVIDER =
            "org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider";

    public static final String MSI_TOKEN_PROVIDER = "org.apache.hadoop.fs.azurebfs.oauth2.MsiTokenProvider";
    ...
    public static final String FS_AZURE_ACCOUNT_OAUTH_PROVIDER_TYPE = "fs.azure.account.oauth.provider.type";
    ...
    public static final String FS_AZURE_IDENTITY_TRANSFORMER_CLASS = "fs.azure.identity.transformer.class";
    ...
    public static final String FS_AZURE_ACCOUNT_OAUTH2_MSI_AUTHORITY = "fs.azure.account.oauth2.msi.authority";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_MSI_ENDPOINT = "fs.azure.account.oauth2.msi.endpoint";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_MSI_TENANT = "fs.azure.account.oauth2.msi.tenant";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_REFRESH_TOKEN = "fs.azure.account.oauth2.refresh.token";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_REFRESH_TOKEN_ENDPOINT =
            "fs.azure.account.oauth2.refresh.token.endpoint";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_TOKEN_FILE = "fs.azure.account.oauth2.token.file";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_USER_NAME = "fs.azure.account.oauth2.user.name";

    public static final String FS_AZURE_ACCOUNT_OAUTH2_USER_PASSWORD = "fs.azure.account.oauth2.user.password";
     ...
     * @param sensitive sensitive flag
     */
    protected void addAccountOption(List<Object[]> list, String key, boolean secret, boolean sensitive) {
        if (!key.isEmpty()) {
    ...
    protected void validateConfig(final Printout printout, final Configuration conf, final boolean writeOperations)
        ...
        int activeBlocks = conf.getInt(FS_AZURE_BLOCK_UPLOAD_ACTIVE_BLOCKS, BLOCK_UPLOAD_ACTIVE_BLOCKS_DEFAULT);
        printout.println(
                "Written data is buffered to %s with up to %d blocks queued per stream", buffering, activeBlocks);
        if ("disk".equals(buffering)) {
            validateBufferDir(printout, conf, FS_AZURE_BLOCK_UPLOAD_BUFFER_DIR, HADOOP_TMP_DIR, writeOperations, 0);
        }
        int leaseThreads;
            ...
            case "SharedKey":
                {
                    ...
                    if (dotIndex <= 0) {
                        ...
                        if (keyProvider.isPresent()) {
                            printout.println(
                                    "Shared Key resolution keys through class %s",
                            ...
                            try {
                                ...
                                if (password.isPresent()) {
                                    ...
                                    getOutput()
                                            .error(
                                                    "No shared key set in %s or %s",
                                                    FS_AZURE_ACCOUNT_KEY, abfsContainerKey(FS_AZURE_ACCOUNT_KEY));
                                }
                            } catch (Exception e) {
                                printout.error(
                                        "Failed to retrieve password configuration option %s: %s",
    ...
    private String[] authorityParts(URI uri) {
        ...
        checkArgument(
                ...
                "Authority is null in \"" + uriText + "\""
                        ...
                        + ABFS_URL_FORM);
        checkArgument(authority.contains("@"), "Authority has no @ splitter \"" + uriText + "\". " + ABFS_URL_FORM);

        final String[] authorityParts = authority.split("@", 2);

        if (authorityParts.length < 2 || authorityParts[0] != null && authorityParts[0].isEmpty()) {
            final String errMsg =
                    String.format("'%s' has a malformed authority, expected container name. " + ABFS_URL_FORM, uriText);
    ...
    public List<EndpointProbe> listEndpointsToProbe(final Printout printout, final Configuration conf)
        ...
        if (store != null) {
            uris.add(new EndpointProbe(
                    String.format("https://%s/", store), "Store", "Determined from filesystem URL", true));
        } else {
     ...
     */
    @Override
    public void preflightFilesystemChecks(final Printout printout, final Path path) throws IOException {

        // look at the host of the account
        final String host = getFsURI().getHost();
        try (StoreDurationInfo ignored = new StoreDurationInfo(LOG, "Probing for account name being a valid host")) {
    ...
    public void validateFilesystem(final Printout printout, final Path path, final FileSystem filesystem)
        ...
        try (StoreDurationInfo ignored =
                new StoreDurationInfo(LOG, "probing for container being classic Azure or ADLS Gen 2 Storage")) {
            ...
            printout.warn(
                    "FileSystem %s returned an error in getIsNamespaceEnabled(): %s",
    ...
    public void validateOutputStream(
            final Printout printout, final FileSystem fs, final Path file, final FSDataOutputStream out)
            throws IOException {
    ...
    private static final class WrappedConfiguration {
        ...
        private WrappedConfiguration(final Configuration conf, final String accountName, final Printout printout) {
            this.conf = conf;
