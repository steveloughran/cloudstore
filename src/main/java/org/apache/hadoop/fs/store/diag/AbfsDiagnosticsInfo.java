...
public class AbfsDiagnosticsInfo extends StoreDiagnosticsInfo {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(AbfsDiagnosticsInfo.class);
  ...
  /**
   * Buffer directory path for uploading AbfsOutputStream data blocks. Value: {@value}
   */
  public static final String FS_AZURE_BLOCK_UPLOAD_BUFFER_DIR = "fs.azure.buffer.dir";

  /**
   * What data block buffer to use. <br>
   * Options include: "disk"(Default), "array", and "bytebuffer". Value: {@value}
   */
  public static final String DATA_BLOCKS_BUFFER = "fs.azure.data.blocks.buffer";
  ...
  /**
   * Limit of queued block upload operations before writes block for an OutputStream. Value:
   * {@value}
   */
  ...
  protected static final Object[][] ENV_VARS =
      {{"AZURE_AUTHORITY_HOST", false}, {"AZURE_CLIENT_ID", false},
          {"AZURE_FEDERATED_TOKEN_FILE", false}, {"AZURE_TENANT_ID", false}, {"", false},};
   ...
   */
  private static final Object[][] OPTIONS = {{"abfs.external.authorization.class", false, false},
      {"fs.abfs.impl", false, false}, {"fs.abfss.impl", false, false},
      {"fs.azure.abfs.endpoint", false, false}, {"fs.azure.abfs.latency.track", false, false},
      {FS_AZURE_ACCOUNT_AUTH_TYPE, false, false}, {FS_AZURE_ACCOUNT_HNS_ENABLED, false, false},
      ...
      {"fs.azure.analysis.period", false, false}, {"fs.azure.shellkeyprovider.script", true, true},
      ...
      {"fs.azure.appendblob.directories", false, false}, {FS_AZURE_ATOMIC_RENAME_KEY, false, false},
      {"fs.azure.block.location.impersonatedhost", false, false},
      {"fs.azure.block.size", false, false}, {"fs.azure.buffered.pread.disable", false, false},
      {"fs.azure.client-provided-encryption-key", true, true},
      {"fs.azure.client.correlationid", false, false}, {"fs.azure.cluster.name", false, false},
      {"fs.azure.cluster.type", false, false}, {"fs.azure.concurrentRequestCount.in", false, false},
      ...
      {"fs.azure.delegation.token.provider.type", false, false}, {DATA_BLOCKS_BUFFER, false, false},
      ...
      {"fs.azure.enable.delegation.token", false, false}, {"fs.azure.enable.flush", false, false},
      {"fs.azure.enable.mkdir.overwrite", false, false}, {FS_AZURE_ENABLE_READAHEAD, false, false},
      ...
      {FS_AZURE_INFINITE_LEASE_DIRECTORIES, false, false}, {"fs.azure.io.rate.limit", false, false},
      ...
      {FS_AZURE_LEASE_THREADS, false, false}, {"fs.azure.list.max.results", false, false},
      ...
      {"fs.azure.readahead.range", false, false}, {FS_AZURE_READAHEADQUEUE_DEPTH, false, false},
      {"fs.azure.rename.raises.exceptions", false, false},
      {FS_AZURE_SAS_TOKEN_PROVIDER_TYPE, false, false}, {"fs.azure.sas.fixed.token", true, true},
      {"fs.azure.sas.token.renew.period.for.streams", false, false},
      {"fs.azure.secure.mode", false, false}, {"fs.azure.shellkeyprovider.script", false, false},
      {"fs.azure.skipUserGroupMetadataDuringInitialization", false, false},
      {"fs.azure.ssl.channel.mode", false, false}, {"fs.azure.tracingheader.format", false, false},
      {"fs.azure.use.upn", false, false}, {"fs.azure.user.agent.prefix", false, false},
      ...
      {"fs.azure.write.request.size", false, false}, {"", false, false},
      ...
      {"fs.azure.abfs.account.name", false, false}, {"fs.contract.test.fs.abfs", false, false},
      {"", false, false},};

  public static final String[] classnames =
      {"com.fasterxml.jackson.annotation.JsonProperty", "com.fasterxml.jackson.core.JsonFactory",
          "com.fasterxml.jackson.databind.ObjectReader", "org.apache.http.client.utils.URIBuilder",
          "org.apache.hadoop.fs.azurebfs.AzureBlobFileSystem",};

  public static final String[] OPTIONAL_CLASSNAMES = {
      // SSL stuff
      "org.wildfly.openssl.OpenSSLProvider", X509,
      // guavae
   ...
   */
  public static final String[] optionalCapabilities = {ETAGS_AVAILABLE, ETAGS_PRESERVED_IN_RENAME,
      FS_ACLS, FS_APPEND, FS_AZURE_CAPABILITY_READAHEAD_SAFE, FS_PERMISSIONS, FS_XATTRS};
  ...
  public Object[][] getFilesystemOptions() {
    ...
    List<Object[]> optionList = new ArrayList<>(Arrays.asList(AbfsDiagnosticsInfo.OPTIONS));
    // dynamically create account-specific keys
    String account = getFsURI().getHost();
    addAccountOption(optionList, FS_AZURE_ACCOUNT_KEY, true, true);
   ...
   */
  protected void add(List<Object[]> list, String key, final boolean secret, boolean sensitive) {
   ...
   * @param sensitive sensitive flag
   */
  protected void addAccountOption(List<Object[]> list, String key, boolean secret,
      boolean sensitive) {
    if (!key.isEmpty()) {
  ...
  protected void validateConfig(final Printout printout, final Configuration conf,
    ...
    int activeBlocks =
        conf.getInt(FS_AZURE_BLOCK_UPLOAD_ACTIVE_BLOCKS, BLOCK_UPLOAD_ACTIVE_BLOCKS_DEFAULT);
    ...
    switch (auth.value) {
          ...
          if (keyProvider.isPresent()) {
            ...
            try {
              ...
              if (password.isPresent()) {
                ...
                printout.println("Secret key for authentication: %s", val.sanitized());
              } else {
                getOutput().error("No shared key set in %s or %s", FS_AZURE_ACCOUNT_KEY,
                    abfsContainerKey(FS_AZURE_ACCOUNT_KEY));
              }
            } catch (Exception e) {
      ...
      case "OAuth":
        ...
        final PropVal provider =
            wrapped.get(FS_AZURE_ACCOUNT_OAUTH_PROVIDER_TYPE).orElse(new PropVal("unset", ""));
        ...
        switch (providerType) {
          ...
          case MSI_TOKEN_PROVIDER:
            passwordStrings = Arrays.asList(FS_AZURE_ACCOUNT_OAUTH2_CLIENT_ID,
                FS_AZURE_ACCOUNT_OAUTH2_MSI_ENDPOINT, FS_AZURE_ACCOUNT_OAUTH2_MSI_TENANT);
            break;
          ...
          case USER_PASSWORD_TOKEN_PROVIDER:
            passwordStrings = Arrays.asList(FS_AZURE_ACCOUNT_OAUTH2_CLIENT_ENDPOINT,
                FS_AZURE_ACCOUNT_OAUTH2_USER_NAME, FS_AZURE_ACCOUNT_OAUTH2_USER_PASSWORD);
            break;
          default:
    ...
    wrapped.printIfDefined("IdentityTransformer", FS_AZURE_IDENTITY_TRANSFORMER_CLASS);
  }
  ...
  public List<EndpointProbe> listEndpointsToProbe(final Printout printout, final Configuration conf)
    ...
    if (store != null) {
      uris.add(new EndpointProbe(String.format("https://%s/", store), "Store",
          "Determined from filesystem URL", true));
    } else {
   ...
   */
  @Override
  public void preflightFilesystemChecks(final Printout printout, final Path path)
      throws IOException {

    // look at the host of the account
    final String host = getFsURI().getHost();
    try (StoreDurationInfo ignored =
        new StoreDurationInfo(LOG, "Probing for account name being a valid host")) {
  ...
  public void validateFilesystem(final Printout printout, final Path path,
    ...
    try (StoreDurationInfo ignored = new StoreDurationInfo(LOG,
      ...
      printout.println("Container %s is an ADLS Gen 2 store", filesystem.getUri());
    } catch (UnsupportedOperationException e) {

      printout.warn("FileSystem %s IS NOT an ADLS Gen 2 store", filesystem.getUri());
  ...
  public void validateOutputStream(final Printout printout, final FileSystem fs, final Path file,
    ...
    try {
      ...
      printout.println("Stream %s lease on the path", hasLease ? "has a" : "has no");
  ...
  private static final class WrappedConfiguration {
    ...
    /**
     * Get a value, returns tuple if found, where first arg is the value, second is source.
     * 
    ...
    private Optional<PropVal> printIfDefined(String text, String key) {
      ...
      val.ifPresent(p -> printout.println("%s = %s", text, p.details()));
      return val;
    }

    private void print(String key, String defVal) {
      printout.println("%s = %s", key, get(key).orElse(new PropVal(defVal, "default")));
    }
