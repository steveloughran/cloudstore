...
public class ADLDiagnosticsInfo extends StoreDiagnosticsInfo {
  ...
  private static final Object[][] options =
      {{"fs.adl.oauth2.access.token.provider.type", false, false},
          ...
          {"fs.adl.oauth2.client.id", true, false}, {"fs.adl.oauth2.credential", true, true},
          {"fs.adl.oauth2.devicecode.clientapp.id", false, false},
          {"fs.adl.oauth2.msi.port", false, false}, {"fs.adl.oauth2.refresh.token", true, true},
  ...
  public static final String[] classnames = {"org.apache.hadoop.fs.adl.AdlFileSystem",
  ...
  public List<EndpointProbe> listEndpointsToProbe(final Printout printout, final Configuration conf)
    ...
    addProbeFromOption(uris, conf, "fs.adl.oauth2.refresh.url", "", "Oauth refresh token endpoint",
        "", true);
    String bucket = getFsURI().getHost();
    uris.add(new EndpointProbe(String.format("https://%s/", bucket), "Store",
        "Determined from filesystem URL", false));
    return uris;
  }
