...
public class ADLDiagnosticsInfo extends StoreDiagnosticsInfo {
    ...
    public static final String[] classnames = {
        "org.apache.hadoop.fs.adl.AdlFileSystem", "com.microsoft.azure.datalake.store.ADLStoreClient",
    };
    ...
    public List<EndpointProbe> listEndpointsToProbe(final Printout printout, final Configuration conf)
        ...
        addProbeFromOption(uris, conf, "fs.adl.oauth2.refresh.url", "", "Oauth refresh token endpoint", "", true);
        String bucket = getFsURI().getHost();
        uris.add(new EndpointProbe(
                String.format("https://%s/", bucket), "Store", "Determined from filesystem URL", false));
        return uris;
    }
