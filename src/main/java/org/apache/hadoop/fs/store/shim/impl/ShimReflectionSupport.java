...
public final class ShimReflectionSupport {
  ...
  /**
   * Convert to an IOE and return for throwing. Wrapper exceptions (invocation, execution) are
   * unwrapped first. If the cause is actually a RuntimeException other than UncheckedIOException or
   * Error, it is thrown
   * 
   * @param e exception
  ...
  /**
   * Convert to an IOE and return for throwing. If the cause is actually a RuntimeException other
   * than UncheckedIOException or Error, it is thrown.
   * 
   ...
   * @return the method or null
   */
  public static <T> Invocation<T> getInvocation(Class<?> source, String name,
   ...
   */
  public static <T> Invocation<T> loadInvocation(Class<?> source, Class<? extends T> returnType,
   ...
   * @return the method or null
   */
  public static <T> Constructor<?> ctor(Class<?> source, Class<?>... parameterTypes) {
    if (source == null) {
      return null;
    }
    try {
   ...
   */
  public static Optional<Method> loadMethodOptional(Class<?> source, String name,
      Class<?>... parameterTypes) {
    return Optional.ofNullable(getMethod(source, name, parameterTypes));
  }

  /**
   * Invoke a method with exception unwrap/uprate. If the method is null, raise
   ...
   */
  public static Object invokeOperation(String operation, Object instance, Method method,
      Object... parameters) throws IOException {
    if (method == null) {
      throw new UnsupportedOperationException("No " + operation + " in " + instance);
    }
    try {
   ...
   */
  public static Object invokeUnchecked(String operation, Object instance, Method method,
      Object... parameters) {
    if (method == null) {
      throw new UnsupportedOperationException("No " + operation + " in " + instance);
    }
  ...
  /**
   * get a list of which features are implemented in the underlying instance.
   * 
   * @param source
  ...
  public static <T> String availability(final IsImplemented source, String... features) {
    ...
    for (String feature : features) {
      result.append(feature).append("=").append(source.isImplemented(feature)).append("\n");
    }
