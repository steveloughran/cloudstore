...
public class JobTokens extends StoreEntryPoint {
    ...
    public static final String USAGE = "Usage: jobtokens [options] <file> <url1> ... <url999>\n"
            + STANDARD_OPTS
    ...
    public int run(String[] args, PrintStream stream) throws Exception {
        ...
        println("Yarn principal for Job Token renewal \"%s\"", renewer == null ? "" : renewer);
        ...
        println("Collecting tokens for %d filesystem%s to to %s", urls.size(), plural(urls.size()), dest);
        Credentials retrieved = self.doAs(
                (PrivilegedExceptionAction<Credentials>) () -> saveTokens(conf, dest, renewer, required, urls));
    ...
    protected Credentials saveTokens(Configuration conf, Path dest, String renewer, boolean required, List<String> urls)
            throws IOException {
        ...
        try (StoreDurationInfo ignored = new StoreDurationInfo(LOG, "Fetching tokens for %s", hosts)) {

            TokenCache.obtainTokensForNamenodes(cred, paths, conf);
            final Collection<Token<? extends TokenIdentifier>> tokens = dumpTokens(cred);

            if (tokens.isEmpty()) {
                println("No tokens collected");
                if (required) {
                    throw new ExitUtil.ExitException(
                            LauncherExitCodes.EXIT_NOT_FOUND, "No tokens collected for hosts " + hosts);
                }
            }
        }
        // all the tokens are collected, so save
        try (StoreDurationInfo ignored = new StoreDurationInfo(LOG, "Saving %d tokens to %s", count, dest)) {
