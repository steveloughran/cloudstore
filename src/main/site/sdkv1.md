...
# Cloudstore support for AWS V1 SDK

There are a set of commands which require the AWS v1 SDK on the classpath; they also make use/abuse of
S3AFileSystem method calls so can be brittle -the `extras` source tree does not compile against older releases of hadoop.

If cloudstore is not built with the `-Pextra` profile, these commands are unavailable.
