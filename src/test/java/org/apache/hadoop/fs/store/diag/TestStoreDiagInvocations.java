...
/**
 * Exercise {@link StoreDiag} in-process against the local filesystem and
 * deliberately broken inputs. None of these tests need cloud credentials.
 *
 * <p>Companion to the contract-test-based suite under
 * {@code org.apache.hadoop.fs.store.contract} which targets each remote
 * filesystem via {@code AbstractFSContractTestBase}.
 */
public class TestStoreDiagInvocations {
    ...
    /**
     * Two positional arguments → CommandFormat rejects before processArgs
     * has a chance to wrap as an ExitException with E_USAGE.
     */
    @Test
    public void testStorediagTooManyArgs() throws Exception {
        intercept(CommandFormat.TooManyArgumentsException.class, () -> StoreDiag.exec("file:///a", "file:///b"));
    }
    ...
    public void testStorediagUnknownScheme() throws Exception {
        intercept(UnsupportedFileSystemException.class, () -> StoreDiag.exec("nosuchfs://example/path"));
    }
    ...
    public void testStorediagMissingRequiredFile() throws Exception {
        ...
        intercept(
                ...
                () -> StoreDiag.exec(
                        "-required", missing.getAbsolutePath(), dir.toURI().toString()));
    }
    ...
    public void testStorediagEmptyRequiredFile() throws Exception {
        ...
        StoreDiag diag = new StoreDiag();
        expectSuccess(
                diag, "-r", "-required", required.getAbsolutePath(), dir.toURI().toString());
    }

    /**
     * Pointing storediag at a path under a nonexistent local directory still
     * succeeds: storediag binds to the filesystem root and reports diagnostics
     * even when the supplied path inside that root is missing. {@code E_NOT_FOUND}
     * only fires when the *root* of the store cannot be resolved, which cannot be
     * triggered against {@code file://} on a host where {@code /} exists.
     */
    @Test
