...
public class TestAuditLogProcessor {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(TestAuditLogProcessor.class);
  ...
  /**
   * A real log entry. This is derived from a real log entry on a test run. If this needs to be
   * updated, please do it from a real log. Splitting this up across lines has a tendency to break
   * things, so be careful making changes.
   */
  static final String SAMPLE_LOG_ENTRY_1 =
      "183c9826b45486e485693808f38e2c4071004bf5dfd4c3ab210f0a21a4000000" + " bucket-london"
          + " [13/May/2021:11:26:06 +0000]" + " 109.157.171.174"
          + " arn:aws:iam::152813717700:user/dev" + " M7ZB7C4RTKXJKTM9" + " REST.PUT.OBJECT"
          + " fork-0001/test/testParseBrokenCSVFile"
          + " \"PUT /fork-0001/test/testParseBrokenCSVFile HTTP/1.1\"" + " 200" + " -" + " -"
          + " 794" + " 55" + " 17" + " \"https://audit.example.org/hadoop/1/op_create/"
          + "e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278/" + "?op=op_create"
          + "&p1=fork-0001/test/testParseBrokenCSVFile" + "&pr=alice"
          + "&ps=2eac5a04-2153-48db-896a-09bc9a2fd132"
          + "&id=e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278&t0=154"
          + "&fs=e8ede3c7-8506-4a43-8268-fe8fcbb510a4&t1=156&" + "ts=1620905165700\""
          + " \"Hadoop 3.4.0-SNAPSHOT, java/1.8.0_282 vendor/AdoptOpenJDK\"" + " -"
          + " TrIqtEYGWAwvu0h1N9WJKyoqM0TyHUaY+ZZBwP2yNf2qQp1Z/0=" + " SigV4"
          + " ECDHE-RSA-AES128-GCM-SHA256" + " AuthHeader"
          + " bucket-london.s3.eu-west-2.amazonaws.com" + " TLSv1.2" + "\n";

  public static final String TIMESTAMP_2 = "13/May/2024:11:26:12 +0000";

  static final String SAMPLE_LOG_ENTRY_2 = "01234567890123456789" + " bucket-london1" + " ["
      + TIMESTAMP_2 + "]" + " 109.157.171.174" + " arn:aws:iam::152813717700:user/dev"
      + " M7ZB7C4RTKXJKTM9" + " REST.PUT.OBJECT" + " fork-0001/test/testParseBrokenCSVFile"
      + " \"PUT /fork-0001/test/testParseBrokenCSVFile HTTP/1.1\"" + " 200" + " -" + " -" + " 794"
      + " 55" + " 17" + " \"https://audit.example.org/hadoop/1/op_create/"
      + "e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278/" + "?op=op_create"
      + "&p1=fork-0001/test/testParseBrokenCSVFile" + "&pr=alice"
      + "&ps=2eac5a04-2153-48db-896a-09bc9a2fd132"
      + "&id=e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278&t0=154"
  ...
  /**
   * A real referrer header entry. This is derived from a real log entry on a test run. If this
   * needs to be updated, please do it from a real log. Splitting this up across lines has a
   * tendency to break things, so be careful making changes.
   */
  static final String SAMPLE_REFERRER_HEADER = "\"https://audit.example.org/hadoop/1/op_create/"
      + "e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278/?" + "op=op_create"
      + "&p1=fork-0001/test/testParseBrokenCSVFile" + "&pr=alice"
      + "&ps=2eac5a04-2153-48db-896a-09bc9a2fd132"
      + "&id=e8ede3c7-8506-4a43-8268-fe8fcbb510a4-00000278&t0=154"
  ...
  /**
   * Testing parseAuditLog method in parser class by passing sample audit log entry and checks if
   * the log is parsed correctly.
   */
  @Test
  public void testParseAuditLogEntry() {
    Map<String, String> parseAuditLogResult = auditLogProcessor.parseAuditLog(SAMPLE_LOG_ENTRY_1);
    assertThat(parseAuditLogResult).describedAs("the result of parseAuditLogResult of %s; %s",
        SAMPLE_LOG_ENTRY_1, auditLogProcessor).isNotNull();
    ...
    assertThat(record.getBucket()).isEqualTo(bucket);
   ...
   */
  private static AbstractStringAssert<?> assertFieldValue(
      final Map<String, String> parseAuditLogResult, final String field, final String expected) {
    return assertThat(parseAuditLogResult.get(field))
        .describedAs("Mismatch in the field %s parsed from the audit", field).isEqualTo(expected);
  }

  /**
   * Testing parseAuditLog method in parser class by passing empty string and null and checks if the
   * result is empty.
   */
  @Test
  public void testParseAuditLogEmptyAndNull() {
    Map<String, String> parseAuditLogResultEmpty = auditLogProcessor.parseAuditLog("");
    assertThat(parseAuditLogResultEmpty).describedAs("Audit log map should be empty").isEmpty();
    Map<String, String> parseAuditLogResultNull = auditLogProcessor.parseAuditLog(null);
  ...
  /**
   * Testing parseReferrerHeader method in parser class by passing sample referrer header taken from
   * sample audit log and checks if the referrer header is parsed correctly.
   */
  @Test
  public void testParseAuditHeader() {
    Map<String, String> parseReferrerHeaderResult = parseAuditHeader(SAMPLE_REFERRER_HEADER);
    // verifying the path 'p1' from parsed referrer header
    assertThat(parseReferrerHeaderResult)
        .describedAs("Mismatch in the path parsed from the referrer").isNotNull()
  ...
  /**
   * Testing parseReferrerHeader method in parser class by passing empty string and null string and
   * checks if the result is empty.
   */
  @Test
  public void testParseAuditEmptyAndNull() {
    assertThat(parseAuditHeader("")).isEmpty();
    assertThat(parseAuditHeader(null)).isEmpty();
  }

  /**
   * Testing mergeAndParseAuditLogFiles method by passing filesystem, source and destination paths.
   */
  @Test
  public void testMergeAndParseAuditLogFiles() throws IOException {
    // Getting the audit dir and file path from test/resources/
    String auditLogDir = this.getClass().getClassLoader().getResource("TestAuditLogs").toString();
    String auditSingleFile =
        this.getClass().getClassLoader().getResource("TestAuditLogs/sampleLog1").toString();
    Preconditions.checkArgument(!StringUtils.isAnyBlank(auditLogDir, auditSingleFile),
        String.format(
            "Audit path should not be empty. Check "
    ...
    long mergeAndParseResult = auditLogProcessor.mergeAndParseAuditLogFiles(auditDirPath, destPath,
        true, org.apache.hadoop.fs.store.audit.AuditLogProcessor.PROCESS_ALL);
    assertThat(mergeAndParseResult)
        .describedAs("The merge and parse failed for the audit log by %s", auditLogProcessor)
        .isGreaterThan(0).isEqualTo(auditLogProcessor.getLogRecordsProcessed());
    ...
    assertThat(auditLogProcessor.getLogFilesParsed())
        .describedAs("log files parsed by %s", auditLogProcessor).isEqualTo(2);
  }
  ...
  /**
   * Testing mergeAndParseAuditLogCounter method by passing filesystem, sample files source and
   * destination paths.
   */
  @Test
  ...
  public void parsesOffsetAndConvertsToUtc() {
    ...
    assertThat(AuditLogProcessor.parseToEpochMillis(s)).describedAs("parse to millis of %s", s)
