...
```bash
hadoop jar cloudstore-1.3.jar cloudup \
...
```bash
 hadoop jar cloudstore-1.3.jar  cloudup -threads 60 -largest 16 -update -hflush -verbose \
...
```
hadoop jar cloudstore-1.3.jar  cloudup -threads 60 -largest 16 -update -block 32  gs://example-london/share/common ./scratch/gcs
...
```bash
 hadoop jar cloudstore-1.3.jar cloudup -threads 100 -largest 32 -block 64 -update share/hadoop/common  s3a://example-london/share/common
...
```bash
 hadoop jar cloudstore-1.3.jar cloudup -threads 120 -largest 32 -block 64 -update share/hadoop/common  s3a://example-london/share/common
...
```bash
bin/hadoop jar cloudstore-1.3.jar cloudup -D fs.s3a.connection.request.timeout=15m -overwrite \
