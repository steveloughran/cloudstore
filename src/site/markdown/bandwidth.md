...
```
> hadoop jar cloudstore-1.2.jar bandwidth
...
```
> hadoop jar cloudstore-1.2.jar bandwidth -csv tmp/s3a128m.csv -block 8 -verbose -policy whole-file 128m s3a://example-london/tmp
...
```
```bash
bin/hadoop jar cloudstore-1.2.jar  bandwidth -D fs.s3a.prefetch.enabled=true -csv tmp/s3a128mp.csv -block 8 -verbose -policy whole-file 128m s3a://example-london
